...
// ── Lengkapi contoh kalimat: spasi antar-bunsetsu + terjemahan ────────────
// Contoh Bab 3-11 lahir dari backfill migration 031 (disalin dari
// `module_grammar.example`) sehingga TANPA spasi dan TANPA terjemahan. Dua-duanya
// dibutuhkan Step 2: tanpa spasi kalimatnya tidak bisa dipotong jadi soal susun
// kalimat, dan tanpa terjemahan soal pilihan ganda kopula jadi ambigu —
// 「私は学生でした。」 sama benarnya dengan kuncinya kalau maksud kalimatnya tidak
// terlihat. Bab 12-20 (081-089) sudah punya keduanya sejak awal.
const PREPARE_SYSTEM = `You segment and translate Japanese example sentences for a JLPT N5/N4 course taught in Indonesian. Reply with plain lines only — no numbering, no extra prose.`;

// Spasi HANYA boleh disisipkan, tidak boleh mengubah satu huruf pun. Dicek
// dengan membandingkan kedua sisi setelah semua spasi dibuang — jadi tidak ada
// jalan bagi model untuk diam-diam menulis ulang kalimat materi.
function spacingOnly(original, spaced) {
  const strip = (t) => String(t || '').replace(/[\s\u3000]+/g, '');
  return strip(original) === strip(spaced) && strip(spaced).length > 0;
}

async function prepareExampleFor(example, pattern) {
  const jp = String(example.japanese || '').trim();
  if (!jp) return null;
  const needSpace = !/[\s\u3000]/.test(jp);
  const needId = !String(example.indonesian || '').trim();
  if (!needSpace && !needId) return null;

  const text = await callClaude({
    system: PREPARE_SYSTEM,
    userContent: `Pola grammar: ${pattern}
Kalimat: ${jp}

Balas TEPAT 2 baris:
Baris 1 — kalimat yang SAMA PERSIS, hanya diberi spasi antar-bunsetsu (satuan
  frasa: kata benda + partikelnya, kata kerja beserta akhirannya). Jangan
  mengubah, menambah, atau menghapus SATU HURUF PUN — hanya menyisipkan spasi.
  Jangan memberi spasi di dalam satu kata. Contoh: 「本を読んで、うちへかえります。」
  menjadi 「本を 読んで、うちへ かえります。」
Baris 2 — terjemahan Bahasa Indonesia yang wajar untuk kalimat itu.`,
    maxTokens: 300,
    model: ANTHROPIC_GEN_MODEL,
  });
  if (text == null) return null;
  ...
  const spaced = needSpace && spacingOnly(jp, lines[0]) ? lines[0] : null;
  const indonesian = needId ? lines[1] : null;
  if (!spaced && !indonesian) return null;
  return { spaced, indonesian };
}

// POST /api/admin/module-grammar/prepare-examples-bulk
// body: { fromGrammarId | courseSlug, limit? } — satu batch, bisa diulang.
router.post('/module-grammar/prepare-examples-bulk', asyncHandler(async (req, res) => {
  if (!anthropicEnabled()) return res.status(503).json({ error: 'ai_disabled' });
  const { fromGrammarId, courseSlug } = req.body || {};
  ...
  // "Belum lengkap" = tidak ada spasi sama sekali, ATAU terjemahannya kosong.
  const pendingSql = `
    SELECT e.id, e.japanese, e.indonesian, g.pattern
      FROM grammar_examples e
      JOIN module_grammar g ON g.id = e.grammar_id
      JOIN modules m ON m.id = g.module_id
     WHERE m.course_id = $1
       AND TRIM(COALESCE(e.japanese, '')) <> ''
       AND (e.japanese !~ '[[:space:]]'
         OR e.indonesian IS NULL OR TRIM(e.indonesian) = '')
     ORDER BY m.sort_order ASC, g.sort_order ASC, e.sort_order ASC`;
  ...
  const failed = [];
  for (const row of batch.rows) {
    const out = await prepareExampleFor(row, row.pattern);
    if (!out) { failed.push(row.japanese); continue; }
    // COALESCE: kolom yang sudah terisi tidak pernah ditimpa.
    await query(
      `UPDATE grammar_examples
          SET japanese = COALESCE($2, japanese),
              indonesian = COALESCE($3, indonesian)
        WHERE id = $1`,
      [row.id, out.spaced, out.indonesian]
    );
    saved++;
  }

  const rest = await query(`SELECT COUNT(*)::int AS n FROM (${pendingSql}) t`, [courseId]);
  res.json({ processed: batch.rows.length, saved, failed, remaining: rest.rows[0].n });
}));
