...
- Navigasi datar 12 tab diganti lima kelompok divisi; semua editor/handler bisnis dipertahankan. Course ID, module ID, lesson ID, slug, konten, soal dan relasi tidak dipindah, disalin, diurutkan ulang atau dibuat ulang oleh pergantian panel.
- Ruang Kerja dapat dibuka admin existing meskipun Company nonaktif. Ringkasan menjelaskan status modul; menu operasional yang berizin tetap dapat dipakai. Gagal mengambil `/company/access` tidak mengunci editor lama, tetapi **tidak** memberikan akses ke modul Company.
...
1. Review diff: tidak ada perubahan pada migration, kontrak schema, backend route bisnis/auth, frontend siswa atau package lock. Editor legacy mulai bagian Live Class dan seterusnya tetap identik dengan baseline merge PR #303.
