-- 062_grammar_task_bab12.sql — Dua Tugas Bunpou Bab 12: Konjugasi Te-form,
-- Menghubungkan Kalimat dengan Te-form.
--
-- Melanjutkan pola tugas bunpou Bab 3-11 (043/046/048/052/054/056/058/060).
-- Konten dari Notion N5-B12 "Te-form: Konjugasi & Penghubung"; modul
-- di-resolve ordinal (OFFSET 11, lanjutan OFFSET 0-10 di 039-061).
--
-- MILESTONE BESAR: te-form adalah konjugasi pertama di luar ます-form yang
-- diajarkan di kurikulum ini — dasar untuk banyak pola lanjutan N5/N4
-- (てください、ています、てもいいです, dst, di luar cakupan migrasi ini).
--
-- 5 pola grammar Bab 12 (GRM-52/53/54/217/218), dibagi 3+2 (pola uneven-
-- split sama seperti Bab 10 yang juga 5 pola, precedent migration 058),
-- di posisi sidebar 5 dan 7 (konvensi sejak Bab 6):
--   Tugas 1 — Konjugasi Te-form (urutan 5, 3 pola): cara membentuk te-form
--     untuk 3 kelompok kata kerja — Golongan 1/u-verbs (う・つ・る→って;
--     ぬ・ぶ・む→んで; く→いて kecuali いく→いって; ぐ→いで; す→して),
--     Golongan 2/ru-verbs (buang る, ganti て), dan 2 kata tidak beraturan
--     (する→して、くる→きて).
--   Tugas 2 — Menghubungkan Kalimat dengan Te-form (urutan 7, 2 pola):
--     〜て、〜 (menghubungkan 2+ aksi berurutan, netral urutan) dan
--     〜てから (setelah ~, urutan tegas X selesai dulu baru Y).
-- required_count = 1 per pola, konsisten dengan tugas bunpou Bab 3-11.
--
-- KANJI: Notion Kanji DB eksplisit "Belum ada kanji yang dilink ke Bab
-- ini" — kandidat te-form (食べる/見る/読む/書く/起きる/寝る/使う/言う)
-- TIDAK official. Sama seperti semua tugas bunpou sebelumnya, contoh
-- module_grammar tetap 100% kana (pola 043/046/048/052/054/056/058/060 —
-- instructional content, bukan quiz_questions ber-pagar-kanji), jadi
-- ketiadaan kanji resmi Bab 12 tidak relevan untuk migrasi ini.
--
-- POSISI: pola PENOMORAN ULANG deterministik yang sama seperti 048/052/
-- 054/056/058/060 (BUKAN geser-relatif) karena menyisip DUA lesson sekaligus.
--     lesson lama ke-1..4  → sort_order 1..4
--     Tugas 1 (Konjugasi Te-form) → sort_order 5
--     lesson lama ke-5     → sort_order 6
--     Tugas 2 (Menghubungkan Kalimat dengan Te-form) → sort_order 7
--     lesson lama ke-6..n  → sort_order 8..n+2
-- Fallback aman: kalau modul Bab 12 punya < 5 lesson lain, penomoran
...
BEGIN
  ...
  IF v_module_id IS NULL THEN
    RAISE NOTICE '062: modul Bab 12 di kursus % tidak ditemukan — skip seed tugas bunpou.', v_course_slug;
    RETURN;
  END IF;

  IF v_module_title !~* '(te-?form|konjugasi|penghubung)' THEN
    RAISE NOTICE '062: modul Bab 12 terbaca "%" — kalau ternyata bukan Bab Te-form, pindahkan pelajarannya lewat admin (tidak perlu migrasi baru).', v_module_title;
  END IF;

  RAISE NOTICE '062: seed 2 Tugas Bunpou Bab 12 ke modul "%".', v_module_title;
  ...
  IF v_g_grup1 IS NULL OR v_g_grup2 IS NULL OR v_g_irreg IS NULL OR v_g_te IS NULL OR v_g_tekara IS NULL THEN
    RAISE EXCEPTION '062: gagal resolve salah satu dari 5 pola grammar Bab 12';
  END IF;
  ...
  IF v_other_count < 5 THEN
    RAISE NOTICE '062: modul "%" cuma punya % lesson lain (< 5) — urutan 5 dan 7 tidak bisa dibentuk, kedua tugas ditaruh di akhir modul. Geser manual lewat admin kalau perlu.',
  ...
  IF (SELECT COUNT(*) FROM lesson_grammar_task_items WHERE lesson_id = v_lesson1_id) <> 3
    ...
    RAISE EXCEPTION '062: Tugas 1 harus punya tepat 3 pola dan Tugas 2 tepat 2 pola (dapat % dan %)',
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '062: ada wiring tanpa instruction atau required_count bukan 1';
  END IF;
  ...
  IF v_other_count >= 5 THEN
    ...
    IF v_pos1 <> 5 OR v_pos2 <> 7 THEN
      RAISE EXCEPTION '062: posisi final tugas bukan 5 dan 7 (dapat % dan %)', v_pos1, v_pos2;
    END IF;
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION '062: ada sort_order kembar di modul setelah penomoran ulang';
    END IF;

    RAISE NOTICE '062: selesai — Tugas Konjugasi Te-form di urutan %, Tugas Menghubungkan Kalimat dengan Te-form di urutan % (5 kalimat wajib total).', v_pos1, v_pos2;
  ELSE
    RAISE NOTICE '062: selesai — 2 tugas ter-wire (5 kalimat wajib total), posisi menyusul diatur manual.';
  END IF;
END $$;
