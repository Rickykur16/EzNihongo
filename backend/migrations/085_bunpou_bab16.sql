...
-- 日火水木金土 (16). 曜 belum diajarkan, jadi contoh menulis ようび dengan kana.
--
    ...
    "examples": [
      {
        "jp": "まいあさ 六時に おきます。",
        "hl": "六時に",
        "id": "Setiap pagi saya bangun jam enam."
      },
      {
        "jp": "日ようびに ともだちと あいます。",
        "hl": "日ようびに",
        "id": "Hari Minggu saya bertemu teman."
      },
      {
        "jp": "九月に 日本へ 行きます。",
        "hl": "九月に",
        "id": "Bulan September saya pergi ke Jepang."
      }
    ]
  },
  {
    "lesson": 1,
    "pattern": "〜月〜日",
    "meaning": "menyatakan tanggal",
    "example": "四月十日",
    "notes": "Bulan = angka + 月 (いちがつ sampai じゅうにがつ). Tanggal 1-10 serta 14, 20, dan 24 punya bacaan khusus (ついたち、ふつか、みっか … とおか、じゅうよっか、はつか、にじゅうよっか); sisanya angka + にち.",
    "examples": [
      {
        "jp": "たんじょうびは 四月十日です。",
        "hl": "四月十日",
        "id": "Ulang tahun saya tanggal 10 April."
      },
      {
        "jp": "十二月二十五日に パーティーが あります。",
        "hl": "十二月二十五日",
        "id": "Tanggal 25 Desember ada pesta."
      },
      {
        "jp": "しけんは 三月一日です。",
        "hl": "三月一日",
        "id": "Ujiannya tanggal 1 Maret."
      }
    ]
  },
  {
    "lesson": 2,
    "pattern": "毎週／毎月／毎年",
    "meaning": "setiap minggu / bulan / tahun",
    "example": "毎週 土ようびに べんきょうします。",
    "notes": "毎 (まい) + satuan waktu berarti \"setiap ~\": まいしゅう、まいつき、まいとし／まいねん、まいにち、まいあさ、まいばん. Kata-kata ini TIDAK memakai partikel に.",
    "examples": [
      {
        "jp": "毎週 土ようびに テニスを します。",
        "hl": "毎週",
        "id": "Setiap Sabtu saya bermain tenis."
      },
      {
        "jp": "毎月 本を 三さつ 読みます。",
        "hl": "毎月",
        "id": "Setiap bulan saya membaca tiga buku."
      },
      {
        "jp": "毎年 うみへ 行きます。",
        "hl": "毎年",
        "id": "Setiap tahun saya pergi ke laut."
      }
    ]
  },
  {
    "lesson": 2,
    "pattern": "何曜日／何月何日／いつ",
    "meaning": "bertanya hari, tanggal, dan waktu",
    "example": "きょうは 何ようびですか。",
    "notes": "何ようび menanyakan hari, 何月何日 menanyakan tanggal, dan いつ menanyakan waktu secara umum (paling luwes, bisa untuk apa saja). Jawaban tanggal dan jam biasanya diikuti です, sedangkan kalimat berkata kerja memakai に.",
    "examples": [
      {
        "jp": "きょうは 何ようびですか。",
        "hl": "何ようび",
        "id": "Hari ini hari apa?"
      },
      {
        "jp": "たんじょうびは 何月何日ですか。",
        "hl": "何月何日",
        "id": "Ulang tahunmu tanggal berapa?"
      },
      {
        "jp": "しけんは いつですか。",
        "hl": "いつ",
...
BEGIN
  ...
  IF v_module_id IS NULL THEN
    RAISE NOTICE '085: modul Bab % di kursus % tidak ditemukan — skip Tata Bahasa.', v_bab_no, v_course_slug;
    RETURN;
  END IF;

  IF v_module_title !~* v_title_re THEN
    RAISE NOTICE '085: modul Bab % terbaca "%" — kalau ternyata bukan bab yang dimaksud, pindahkan pelajarannya lewat admin (tidak perlu migrasi baru).',
      v_bab_no, v_module_title;
  END IF;

  RAISE NOTICE '085: seed 2 pelajaran Tata Bahasa Bab % ke modul "%".', v_bab_no, v_module_title;
  ...
  IF EXISTS (
    ...
    RAISE NOTICE '085: modul "%" sudah punya pelajaran bertipe text lain (%) — cek duplikasi materi Tata Bahasa lewat admin.',
      v_module_title,
  ...
  IF v_other < 3 THEN
    RAISE NOTICE '085: modul "%" cuma punya % lesson lain (< 3) — urutan 4 dan 6 tidak bisa dibentuk, kedua pelajaran ditaruh di akhir modul. Geser manual lewat admin kalau perlu.',
  ...
  IF (SELECT COUNT(*) FROM module_grammar WHERE lesson_id = v_l1) <> v_n_l1
    ...
    RAISE EXCEPTION '085: jumlah pola harus % dan % (dapat % dan %)',
      v_n_l1, v_n_l2,
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '085: ada pola dengan < 2 contoh kalimat atau contoh tanpa terjemahan';
  END IF;

  -- Pagar level: contoh kalimat hanya boleh memakai kanji yang sudah
  -- diajarkan sampai Bab 16 (lihat v_kanji_ok).
  IF EXISTS (
    ...
    RAISE EXCEPTION '085: ada kanji di luar daftar taught (s/d Bab %) pada contoh kalimat', v_bab_no;
  END IF;
  ...
  IF v_other >= 3 THEN
    ...
    IF v_pos1 <> 4 OR v_pos2 <> 6 THEN
      RAISE EXCEPTION '085: sort_order final pelajaran bukan 4 dan 6 (dapat % dan %)', v_pos1, v_pos2;
    END IF;
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION '085: ada sort_order kembar di modul setelah penomoran ulang';
    END IF;

    RAISE NOTICE '085: selesai — "%" di sort_order 4, "%" di sort_order 6, % pola / % contoh (slot 5 & 7 untuk Tugas Bunpou: % terisi).',
  ...
  ELSE
    RAISE NOTICE '085: selesai — % pola ter-wire, posisi menyusul diatur manual.', v_n_l1 + v_n_l2;
  END IF;
END $mig$;
