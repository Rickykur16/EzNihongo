-- 077_bab19_intro_kosakata_kanji.sql — Struktur dasar Bab 19
-- (Keinginan & Rencana): Pelajaran 1 Pengantar (video), Pelajaran 2
...
-- Notion. Ringkasnya:
--   * modul di-resolve ordinal (OFFSET 18, sama seperti 039-065); judul yang
...
--     Bab dari Notion" (N5-B19, 32 kata di Notion);
--   * kanji: 5 karakter baru (雨・天・空・山・川);
...
DECLARE
  ...
  v_bab_no        INT  := 19;
  v_kode          TEXT := 'N5-B19';
  v_mod_slug      TEXT := 'keinginan-rencana';
  v_mod_title     TEXT := 'Keinginan & Rencana';
  v_mod_title_en  TEXT := 'Wants & Plans';
  v_mod_cefr      TEXT := 'A2';
  v_mod_topic     TEXT := 'Self & Others';
  v_mod_scenario  TEXT := 'Membicarakan rencana dan cita-cita: studi, karier, visa.';
  v_mod_cando     JSONB := '["Mengungkapkan keinginan pribadi dan akademis/profesional", "Berbicara tentang rencana masa depan", "Membahas cita-cita dan target"]'::jsonb;
...
BEGIN
  ...
  IF v_course_id IS NULL THEN
    RAISE NOTICE '077: kursus % tidak ditemukan — skip Bab %.', v_course_slug, v_bab_no;
    RETURN;
  END IF;
  ...
  IF v_module_id IS NOT NULL AND v_module_title !~* v_title_re THEN
    RAISE NOTICE '077: modul Bab % terbaca "%" — kalau ternyata bukan bab yang dimaksud, pindahkan pelajarannya lewat admin (tidak perlu migrasi baru).',
  ...
  IF v_module_id IS NULL THEN
    ...
    RAISE NOTICE '077: modul Bab % belum ada — dibuat (slug %, sort_order %).',
  ...
  ELSE
    ...
    RAISE NOTICE '077: pakai modul "%" untuk Bab %.', v_module_title, v_bab_no;
  END IF;
  ...
  VALUES (v_module_id, 'pelajaran-1-pengantar', 'Pelajaran 1: Pengantar', 'video',
          $html$<p>Pengantar Bab 19 — Keinginan &amp; Rencana. Tonton video pembuka, lalu lanjut ke Kosakata dan Kanji.</p>
<p><strong>Struktur bab:</strong> Pengantar → Kosakata → Kanji → Tata Bahasa → Latihan → Kuis.</p>$html$, 1, 10)
  ...
  -- dari Notion" → pilih N5-B19. Upsert tombol itu identik dengan
  ...
  -- === 4. Kanji ===========================================================
  CREATE TEMP TABLE _b19_kanji (character TEXT, on_reading TEXT, kun_reading TEXT, meaning_id TEXT, stroke_count INT, mnemonic TEXT, ord INT) ON COMMIT DROP;
  INSERT INTO _b19_kanji VALUES
    ('雨', 'ウ', 'あめ', 'hujan', 8,
     NULL, 1),
    ('天', 'テン', 'あま', 'langit, surga', 4,
     NULL, 2),
    ('空', 'クウ', 'そら、あ(く)、から', 'langit, kosong', 8,
     NULL, 3),
    ('山', 'サン', 'やま', 'gunung', 3,
     NULL, 4),
    ('川', 'セン', 'かわ', 'sungai', 3,
  ...
  RAISE NOTICE '077: Bab % siap — Pengantar (video) + Kosakata (% kata) + Kanji (% karakter).',
