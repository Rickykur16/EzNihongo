-- 064_bab15_intro_kosakata_kanji.sql — Struktur dasar Bab 15
...
-- Notion. Ringkasnya:
--   * modul di-resolve ordinal (OFFSET 14) → cari by judul → dibuat kalau
...
BEGIN
  ...
  IF v_course_id IS NULL THEN
    RAISE NOTICE '064: kursus % tidak ditemukan — skip Bab %.', v_course_slug, v_bab_no;
    RETURN;
  END IF;
  ...
  IF v_module_id IS NOT NULL AND v_module_title !~* v_title_re THEN
    RAISE NOTICE '064: modul ordinal ke-% berjudul "%" tidak cocok pola Bab % — coba cari by judul.',
  ...
  IF v_module_id IS NULL THEN
    ...
    RAISE NOTICE '064: modul Bab % belum ada — dibuat (slug %, sort_order %).',
  ...
  ELSE
    ...
    RAISE NOTICE '064: pakai modul "%" untuk Bab %.', v_module_title, v_bab_no;
  END IF;
...
<p><strong>Struktur bab:</strong> Pengantar → Kosakata → Kanji → Tata Bahasa → Latihan → Kuis.</p>$html$, 1, 10)
  ...
  RAISE NOTICE '064: Bab % siap — Pengantar (video) + Kosakata (% kata) + Kanji (% karakter).',
