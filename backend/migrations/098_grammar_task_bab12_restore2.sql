-- 098_grammar_task_bab12_restore2.sql — Restore (KEDUA KALINYA) 2 Tugas
-- Bunpou Bab 12 (Konjugasi Te-form, Menghubungkan Kalimat dengan Te-form) —
-- user melaporkan lesson-nya terhapus lagi setelah restore pertama
-- (065_grammar_task_bab12_restore.sql) sempat live di production.
--
-- Migration 062 dan 065 sudah ter-apply & tercatat di schema_migrations,
-- jadi TIDAK bisa dijalankan ulang oleh runner (pola sama seperti 044/063
-- yang memperbaiki migrasi sebelumnya) — file baru ini menjalankan LOGIKA
-- YANG PERSIS SAMA dengan 062/065 (idempotent by design: lesson di-upsert
-- per module_id+slug — kalau baris sudah terhapus, INSERT biasa akan
...
-- hilang.
--
-- CATATAN: kalau lesson ini terhapus untuk KETIGA kalinya, curigai ada
-- proses/orang yang menghapusnya sengaja atau tidak sengaja lewat admin
-- secara rutin — cek dulu sebelum restore lagi, jangan cuma re-run migrasi
-- ini terus-menerus.
--
-- Konten SAMA PERSIS dengan 062/065 — lihat 062_grammar_task_bab12.sql
-- untuk detail rasional 5 pola/split 3+2/posisi 5&7. Tidak ada perubahan
-- desain di sini, murni restore kedua.
...
BEGIN
  ...
  IF v_module_id IS NULL THEN
    RAISE NOTICE '098: modul Bab 12 di kursus % tidak ditemukan — skip restore.', v_course_slug;
    RETURN;
  END IF;

  IF v_module_title !~* '(te-?form|konjugasi|penghubung)' THEN
    RAISE NOTICE '098: modul Bab 12 terbaca "%" — kalau ternyata bukan Bab Te-form, pindahkan pelajarannya lewat admin (tidak perlu migrasi baru).', v_module_title;
  END IF;

  RAISE NOTICE '098: restore (kedua kalinya) 2 Tugas Bunpou Bab 12 ke modul "%".', v_module_title;
  ...
  IF v_g_grup1 IS NULL OR v_g_grup2 IS NULL OR v_g_irreg IS NULL OR v_g_te IS NULL OR v_g_tekara IS NULL THEN
    RAISE EXCEPTION '098: gagal resolve salah satu dari 5 pola grammar Bab 12';
  END IF;
  ...
  IF v_other_count < 5 THEN
    RAISE NOTICE '098: modul "%" cuma punya % lesson lain (< 5) — urutan 5 dan 7 tidak bisa dibentuk, kedua tugas ditaruh di akhir modul. Geser manual lewat admin kalau perlu.',
  ...
  IF (SELECT COUNT(*) FROM lesson_grammar_task_items WHERE lesson_id = v_lesson1_id) <> 3
    ...
    RAISE EXCEPTION '098: Tugas 1 harus punya tepat 3 pola dan Tugas 2 tepat 2 pola (dapat % dan %)',
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '098: ada wiring tanpa instruction atau required_count bukan 1';
  END IF;
  ...
  IF v_other_count >= 5 THEN
    ...
    IF v_pos1 <> 5 OR v_pos2 <> 7 THEN
      RAISE EXCEPTION '098: posisi final tugas bukan 5 dan 7 (dapat % dan %)', v_pos1, v_pos2;
    END IF;
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION '098: ada sort_order kembar di modul setelah penomoran ulang';
    END IF;

    RAISE NOTICE '098: selesai — Tugas Konjugasi Te-form di urutan %, Tugas Menghubungkan Kalimat dengan Te-form di urutan % (5 kalimat wajib total).', v_pos1, v_pos2;
  ELSE
    RAISE NOTICE '098: selesai — 2 tugas ter-wire (5 kalimat wajib total), posisi menyusul diatur manual.';
  END IF;
END $$;
