-- 049_diagnose_kanji_items.sql — Diagnostic dump kanji_items (READ-ONLY,
...
BEGIN
  ...
  IF v_course_id IS NULL THEN
    RAISE NOTICE '049: course n5 tidak ditemukan — skip diagnostic.';
    RETURN;
  END IF;
  ...
  RAISE NOTICE '049: total kanji_items N5 = %, orphan (lesson_id NULL) = %', v_total, v_orphan;

  RAISE NOTICE '049: ==== modul n5, urutan ordinal (Bab 1 = OFFSET 0, dst) ====';
  FOR r IN
  ...
  LOOP
    RAISE NOTICE '049: Bab % -> module_id=% slug=% title=%', r.bab_no, r.id, r.slug, r.title;
  END LOOP;

  RAISE NOTICE '049: ==== 6 kanji resmi Bab 6 (高安新古長白) — status saat ini ====';
  FOREACH v_ch IN ARRAY v_bab6_chars LOOP
    IF NOT EXISTS (SELECT 1 FROM kanji_items WHERE character = v_ch AND jlpt_level = 'N5') THEN
      RAISE NOTICE '049: !! kanji % TIDAK ADA SAMA SEKALI di kanji_items (N5) !!', v_ch;
    END IF;
  END LOOP;
  ...
  LOOP
    RAISE NOTICE '049: kanji=% -> Bab % (module=%/%) lesson=%/% lesson_id=% bab_kode=% sort=% created_at=% updated_at=%',
  ...
  RAISE NOTICE '049: ==== seluruh kanji_items N5, dikelompokkan per Bab aktual (ordinal modul) ====';
  FOR r IN
  ...
  LOOP
    RAISE NOTICE '049: Bab % (%) kanji=% bab_kode=% lesson_id=% updated_at=%',
      r.bab_no, r.module_slug, r.character, r.bab_kode, r.lesson_id, r.updated_at;
  END LOOP;

  RAISE NOTICE '049: ==== bab_kode yang kanji-nya SEKARANG tersebar di > 1 modul (indikasi tercuri) ====';
  FOR r IN
  ...
  LOOP
    RAISE NOTICE '049: !! bab_kode=% tersebar di % modul (%) — detail: %',
      r.bab_kode, r.n_modul, r.moduls, r.detail;
  END LOOP;

  RAISE NOTICE '049: selesai — diagnostic read-only, tidak ada perubahan data.';
END $$;
