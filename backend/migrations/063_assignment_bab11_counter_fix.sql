...
-- Counter (Penghitung), isi celah bacaan kanji 年 yang belum pernah diuji.
--
...
--
-- SEBAB: user menunjukkan screenshot kanji lesson production Bab 11 —
-- kanji resmi Bab 11 cuma 8: 年百午半週万千毎 (SAMA dengan yang sudah
-- dikonfirmasi dari Notion Kanji DB First Lesson=N5-B11 sebelumnya, tidak
-- ada kata bantu bilangan 個/枚/匹/dst — 061 yang asli SUDAH BENAR soal
-- itu, revert migration 063 sebelumnya yang salah premis).
--
-- SETELAH DICEK ULANG: dari 8 kanji itu, 週/毎 sudah diuji di 061 (soal
-- 1-3, kombinasi 先週/毎週/毎月), dan 百/千/万/半/午 sudah diuji DI BAB 5
-- (migration 047 — 三百円/六百円/八百円, 千円, 万円, 半, 午前/午後), jadi
-- tidak perlu diulang. TAPI 年 cuma masuk whitelist "dianggap taught"
-- sejak Bab 5 (rombongan 時分円百千万年月半歳午前後, lihat header 047)
-- dan TIDAK PERNAH punya soal yang menguji bacaannya secara langsung, dari
-- Bab 5 sampai Bab 11 — celah asli, bukan kesalahpahaman kali ini.
--
-- FIX: soal 9 (漢字読み, tadinya 三十人＝さんじゅうにん — bacaan reguler,
-- kurang penting untuk diulang) diganti dengan 一年＝いちねん. 年 tidak
-- whitelist BARU (sudah taught sejak Bab 5, TIDAK perlu ubah pagar kanji
-- sama sekali) — murni ganti 1 dari 9 soal もんだい1 漢字読み. §2/§3/§4
-- TIDAK berubah dari 061.
--
-- REF_CHECK BARU (menggantikan 三十人＝さんじゅうにん di soal 9):
--   一年＝いちねん (beraturan, TIDAK ada perubahan bunyi — kontras dengan
--   一本＝いっぽん yang berubah bunyi, catatan pedagogis di explanation).
--
-- DEDUP: TIDAK berubah dari 061 (masih menolak 114 target dari 042/045/
-- 047/051/053/055/057/059). Target BARU migrasi ini sendiri (一年) WAJIB
-- masuk DEDUP_PRIOR migrasi assignment Bab 12+ berikutnya.
--
-- KANJI/PARTIKEL/KATA KERJA: TIDAK berubah dari 061 (whitelist 67 kanji
-- sama persis, tidak ada assertion partikel, verb strip-chain sama —
-- あります／います + おねがいします; soal 9 baru pakai いました yang
-- sudah ada di strip-chain).
--
...
BEGIN
  ...
  RAISE NOTICE '063: perbaiki Assignment Bab 11 (isi celah bacaan 年) di modul "%".', v_module_title;
  ...
  WITH sect(cat, num, label, instruction) AS (VALUES
    ...
    -- ===== もんだい1 漢字読み (1-9) — kombinasi 週/毎/年 + perubahan bunyi 本/人 =====
    ...
    (9, 'vocabulary', 1, 'にほんに <u>一年</u> いました。',
        '一年 dibaca いちねん (satu tahun). 一 dibaca いち, 年 dibaca ねん (onyomi) saat digabung dengan angka — beda dengan 一本＝いっぽん, di sini 年 TIDAK mengalami perubahan bunyi (tetap ねん).'),
  ...
  WITH o(qno, ord, option_text, ok) AS (VALUES
    ...
    (9, 0, 'いちねん', TRUE), (9, 1, 'いちどし', FALSE), (9, 2, 'いってん', FALSE), (9, 3, 'ひとねん', FALSE),
  ...
  -- ===== Pagar level (SAMA PERSIS dengan 061 — tidak ada kanji/partikel/
  -- verba baru yang perlu ditambah, 年 sudah taught sejak Bab 5) =====
  ...
  --    yang masih dilarang. Soal 9 baru pakai いました — sudah ada di
  --    strip-chain (tidak perlu entry baru).
  IF EXISTS (
    SELECT 1 FROM quiz_questions
     WHERE lesson_id = v_lesson_id
  ...
  RAISE NOTICE '063: selesai — 50 soal (vocabulary 30, grammar 20), semua pagar level + dedup lolos, celah bacaan 年 sudah diisi.';
END $$;
