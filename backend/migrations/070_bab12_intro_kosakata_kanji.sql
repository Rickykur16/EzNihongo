-- 070_bab12_intro_kosakata_kanji.sql — Struktur dasar Bab 12
...
-- Notion. Ringkasnya:
--   * modul di-resolve ordinal (OFFSET 11, sama seperti 039-065); judul yang
...
BEGIN
  ...
  IF v_course_id IS NULL THEN
    RAISE NOTICE '070: kursus % tidak ditemukan — skip Bab %.', v_course_slug, v_bab_no;
    RETURN;
  END IF;
  ...
  IF v_module_id IS NOT NULL AND v_module_title !~* v_title_re THEN
    RAISE NOTICE '070: modul Bab % terbaca "%" — kalau ternyata bukan bab yang dimaksud, pindahkan pelajarannya lewat admin (tidak perlu migrasi baru).',
  ...
  IF v_module_id IS NULL THEN
    ...
    RAISE NOTICE '070: modul Bab % belum ada — dibuat (slug %, sort_order %).',
  ...
  ELSE
    ...
    RAISE NOTICE '070: pakai modul "%" untuk Bab %.', v_module_title, v_bab_no;
  END IF;
...
<p><strong>Struktur bab:</strong> Pengantar → Kosakata → Kanji → Tata Bahasa → Latihan → Kuis.</p>$html$, 1, 10)
  ...
  RAISE NOTICE '070: Bab % siap — Pengantar (video) + Kosakata (% kata) + Kanji (% karakter).',
