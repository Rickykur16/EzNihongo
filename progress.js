(() => {
  ...
  function chapter(item) {
    ...
    return `<article class="chapter"><div class="chapter-top"><div><h3>${esc(item.title)}</h3><span class="muted">${esc(item.section || 'Kurikulum')}</span></div><strong>${item.progress.percentage}% selesai</strong></div><div class="chapter-meta"><span class="pill">${item.progress.completedLessons}/${item.progress.totalLessons} Pelajaran</span><span class="pill">${performance.percentage == null ? 'Belum cukup data' : `${esc(performance.label)} · ${performance.percentage}%`}</span>${item.reviewDue ? '<a class="pill" href="review.html">' + item.reviewDue + ' item review</a>' : ''}</div></article>`;
  }
  ...
  function render(data) {
    ...
    app.innerHTML = `<div class="page"><section class="hero"><div><div class="eyebrow">PROGRESS</div><h1>${esc(course.title)}</h1><p class="muted">Course Progress adalah penyelesaian kurikulum. Performa berasal dari bukti latihan.</p></div>${data.courses?.length > 1 ? `<label class="course-switch"><span>Kelas aktif</span><select id="course" class="course-select" aria-label="Pilih kelas">${data.courses.map((item) => `<option value="${esc(item.slug)}" ${item.id === course.id ? 'selected' : ''}>${esc(item.title)}</option>`).join('')}</select></label>` : ''}</section><section class="grid"><article class="card"><div class="eyebrow">COURSE PROGRESS</div><div class="number">${course.progress.percentage}%</div><p class="muted">${course.progress.completedLessons} dari ${course.progress.totalLessons} Pelajaran selesai.</p></article><article class="card"><div class="eyebrow">AKTIVITAS 7 HARI</div><h2>${activity.activeDays || 0} hari aktif</h2><p class="muted">${activity.lessonsCompleted || 0} Pelajaran · ${activity.reviewQuestions || 0} review · ${activity.accuracy == null ? 'Akurasi belum cukup data' : `${activity.accuracy}% akurasi`}</p></article></section><section class="card"><div class="eyebrow">PERFORMA</div><h2>Mastery berdasarkan bukti latihan</h2><div class="mastery">${Object.entries(labels).map(([key]) => mastery(key, masteryData[key])).join('')}</div></section><section class="card trend-card"><div class="eyebrow">AKTIVITAS & TREND</div><p>${esc(trendText(activity))}</p><p class="muted">${esc(data.weeklyInsight?.message || 'Lanjutkan dengan ritme belajar yang nyaman.')}</p>${data.review?.total ? `<a class="btn" href="review.html">Mulai Smart Review (${data.review.total})</a>` : ''}</section><section class="sections"><div><div class="eyebrow">PER BAB</div><h2>Progress dan kebutuhan review</h2></div>${[...grouped.entries()].map(([section, items]) => `<section><h3 class="section-title">${esc(section)}</h3><div class="chapters">${items.map(chapter).join('')}</div></section>`).join('') || '<div class="empty">Belum ada Bab pada kelas ini.</div>'}</section></div>`;
