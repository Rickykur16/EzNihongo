...
test('rollback restores exact previous bytes and removes only newly created Operations config', t => {
  const dir = mkdtempSync(path.join(tmpdir(), 'eznihongo-operations-config-test-'));
  t.after(() => rmSync(dir, { recursive: true, force: true }));
  const previous = path.join(dir, 'existing.env');
  const added = path.join(dir, 'operations.conf');
  const unrelated = path.join(dir, 'unrelated.env');
  writeFileSync(previous, 'STUDENT_OPERATIONS_ENABLED=false\nSENTINEL=preserved\n');
  ...
  restore();
  assert.equal(readFileSync(previous, 'utf8'), 'STUDENT_OPERATIONS_ENABLED=false\nSENTINEL=preserved\n');
...
test('scratch database name cannot equal production or contain injected SQL', () => {
  ...
  assert.equal(name, 'eznihongo_operations_restore_0123456789abcdef');
